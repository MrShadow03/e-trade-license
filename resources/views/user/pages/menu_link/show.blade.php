...
<!--begin::Page Title-->
@section('title')
    <title>Menu Mangaer | {{ $menu->title }} | Admin</title>
@endsection
<!--end::Page Title-->
                ...
                <div class="d-flex align-items-start">
                    ...
                    <div class="symbol symbol-circle me-5">
                        <div class="symbol-label bg-transparent text-primary border border-secondary border-dashed">
                            <i class="ki-duotone ki-element-11 fs-2x" style="color: {{ $menu->color ?? 'ffff' }}">
                    ...
                    <div class="d-flex flex-column">
                        <h2 class="font-bn" style="color: {{ $menu->color ?? 'ffff' }}">
                            {{ $menu->title }}
                        </h2>
                        <div class="text-gray-600 fs-5 font-noto">{{ App\Helpers\Helpers::convertToBanglaDigits($menu->links()->count()) }} টি লিংক</div>
                        <ol class="breadcrumb text-muted fs-6 fw-semibold mt-4">
                            <li class="breadcrumb-item text-muted"><a href="{{ route('admin.menu-link') }}" class="text-muted text-hover-primary text-hover-underline">মেনু সমূহ</a></li>
                            <li class="breadcrumb-item text-muted">{{ $menu->title }}</li>
                        </ol>                        
                    </div>
                    <!--end::Title-->
                </div>
                @can('create-menu-link')
                <button type="button" class="btn btn-flex font-bn text-white" style="background-color: {{ $menu->color ?? 'ffff' }}" data-bs-toggle="modal" data-bs-target="#modal_add_link">
        ...
        <div class="card-body">
            ...
            <table id="kt_file_manager_list" data-kt-filemanager-table="files" class="table align-middle table-row-dashed fs-6 gy-5 font-bn">
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach ($menu->links->groupBy('category') as $category => $links)
                    @if ($category)
                    <tr class="bg-light-primary bg-opacity-10">
                        <td class="py-0" colspan="3">
                            <div class="separator separator-content my-15"><span style="color: {{ $menu->color }}">{{ $category }}</span></div>
                        </td>
                    </tr>
                    @endif
                                ...
                                <div class="ms-2">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-150px py-4" data-kt-menu="true">
                                        ...
                                        <div class="menu-item px-3">
                                            <a 
                                            @if($link->type == 'pdf')
                                            href="{{ asset('storage').'/'.$link->doc_path }}"
                                            @elseif($link->type == 'image')
                                            href="{{ asset('storage').'/'.$link->image }}"
                                            @endif
           ...
           <!--begin::Modal header-->
            ...
            <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                ...
                <form id="modal_new_targ_banner" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                    action="{{ route('admin.menu-link.link.store', $menu->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <!--begin::Input group-->
                    ...
                    <div class="d-flex flex-column mb-4 fv-row fv-plugins-icon-container">
                        ...
                        <select class="form-select" id="newLinkCategoryId" name="category" data-dropdown-parent="#modal_add_link">
                            <option value="">কোনো ক্যাটেগরি নেই</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category }}" @selected($latest_link ? $latest_link->category === $category : false) >{{ $category }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!--end::Input group-->
            ...
            <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                ...
                <form id="modal_new_targ_banner" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                    ...
                    <div class="d-flex flex-column mb-4 fv-row fv-plugins-icon-container">
                        ...
                        <select class="form-select" id="linkUpdateCategory" name="category" data-dropdown-parent="#modal_update_link">
                            <option value="">কোনো ক্যাটেগরি নেই</option>
                            @foreach ($categories as $category)
...
@canany(['create-menu-link', 'edit-menu-link', 'delete-menu-link'])
@section('exclusive_scripts')
    <script>
        ...
        function inputPageData(data, assetPath){
            ...
            linkUpdateCategory.value = data.category;
            // update select2
            $('#linkUpdateCategory').val(data.category).trigger('change');
