        ...
        <div class="card-header pt-10">
            <div class="d-flex align-items-center">
                ...
                <div class="d-flex flex-column">
                    <h2 class="font-bn">গুরুত্বপূর্ণ লিংকসমূহ</h2>
                    <div class="text-muted">
                        {{ App\Helpers\Helpers::convertFileSize($all_tos->sum('size')) }} <span class="mx-3">|</span> {{ $all_tos->count() }} টি লিংক
                    </div> 
                </div>
                <!--end::Title-->
            </div>
        ...
        <div class="card-header pt-8 border-bottom">
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-kt-filemanager-table-filter="search" class="form-control w-250px ps-15 font-bn" placeholder="লিংক সার্চ করুন" />
                </div>
                <!--end::Search--> 
            </div>
            ...
            <div class="card-toolbar">
                ...
                <div class="d-flex justify-content-end" data-kt-filemanager-table-toolbar="base">
                    @can('create-tos')
                    <!--begin::Add customer-->
                    <button type="button" class="btn btn-flex btn-primary font-bn text-white" data-bs-toggle="modal"
                    data-bs-target="#modal_add_link">
                    ...
                    <tr>
                        <td class="py-1">
                            <div class="d-flex align-items-center">
                                ...
                                <a 
                                @if ($tos->type == 'article')
                                href="{{ route('admin.link.edit', $tos->id) }}"
                                @elseif ($tos->type == 'pdf' || $tos->type == 'image')
                                href="javascript:void(0);" 
                                onclick="inputPageData({{ json_encode($tos) }}, '{{ asset('storage') }}')"
                                ...
                                @endif
                                @if ($tos->type === 'article' && (!$tos->article))
                        ...
                        <td class="py-1 font-bn" data-order="{{ $tos->size }}">{{ App\Helpers\Helpers::convertFileSize($tos->size) }}</td>
                        <td class="py-1 font-bn">
                            {{ $tos->updated_at->format('d M Y, h:i a') }}
                        </td>
                        <td class="text-end py-1" data-kt-filemanager-table="action_dropdown">
                            <div class="d-flex justify-content-end">
                                ...
                                <div class="ms-2">
                                    @can('edit-tos')
                                    @if($tos->type == 'article')
                                        <a href="{{ route('admin.link.edit', $tos->id) }}" class="btn btn-sm btn-icon btn-light-info" target="_self" title="পরিবর্তন করুন">
                                            <i class="ki-solid ki-feather fs-3 m-0">
                                            </i>
                                        </a>
                                    @elseif($tos->type == 'pdf' || $tos->type == 'image' || $tos->type == 'url')
                                    ...
                                    @endif
                                    <form 
                                    action="{{ route('admin.link.visibility', $tos->id) }}"  
                                    class="btn btn-sm btn-icon 
                                    {{ $tos->visibility ? 'btn-light-primary' : 'btn-secondary' }}"  
                                    method="POST"
                                    onclick="this.submit()"
                                    title="{{ $tos->visibility ? 'ওয়েবসাইট থেকে সরিয়ে ফেলুন' : 'ওয়েবসাইটে যোগ করুন' }}"
                                    >
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    @can('delete-tos')
                                    <form 
                                    action="{{ route('admin.link.destroy', $tos->id) }}"  
                                    class="btn btn-sm btn-icon btn-light-danger" 
                                    method="POST"
                                    onclick="submitForm(this, event, '{{ $tos->name }}')"
                                    title="ডিলিট করুন"
                                    >
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="modal-content rounded">
            ...
            <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                ...
                <form id="modal_new_targ_banner" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                    action="{{ route('admin.tos.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <!--begin::Input group-->
            ...
            <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                ...
                <form id="modal_new_targ_banner" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                    ...
                    <div class="d-flex flex-column mb-4 fv-row fv-plugins-icon-container">
                        ...
                        <input type="date" id="linkUpdateStartDate" class="form-control" name="date" required>
                        <div class="fv-plugins-message-container invalid-feedback"></div>
                    </div>
                    <!--end::Input group-->
...
<script src="{{ asset('./assets/admin/assets/plugins/custom/datatables/datatables.bundle.js') }}"></script>
    ...
    <script>
        ...
        function submitForm(form, event, title){
            ...
            Swal.fire({
                title: `আপনি কি \"${title}\" লিংক ডিলিট করতে চান?`,
        ...
        function inputPageData(data, assetPath){
            ...
            linkUpdateStartDate.value = data.linkable.date.split(' ')[0];
