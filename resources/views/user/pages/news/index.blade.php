        ...
        <div class="card-header pt-10">
            <div class="d-flex align-items-center">
                ...
                <div class="d-flex flex-column">
                    <h2 class="font-bn">সকল বার্তা</h2>
                    <div class="text-muted">
                        {{ App\Helpers\Helpers::convertFileSize($all_news->sum('size')) }} <span class="mx-3">|</span> {{ $all_news->count() }} টি বার্তা
                    </div> 
                </div>
                <!--end::Title-->
            </div>
        ...
        <div class="card-header pt-8 border-bottom">
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-kt-filemanager-table-filter="search" class="form-control w-250px ps-15 font-bn" placeholder="বার্তা সার্চ করুন" />
                </div>
                <!--end::Search--> 
            </div>
                    ...
                    <tr>
                        <td class="py-1">
                            <div class="d-flex align-items-center">
                                ...
                                href="{{ route('admin.link.edit', $news->id) }}"
                                @elseif ($news->type == 'pdf' || $news->type == 'image')
                                href="javascript:void(0);" 
                                onclick="inputPageData({{ json_encode($news) }}, '{{ asset('storage') }}')"
                                ...
                                @endif
                                @if ($news->type === 'article' && (!$news->article))
                        ...
                        <td class="py-1 font-bn" data-order="{{ $news->size }}">{{ App\Helpers\Helpers::convertFileSize($news->size) }}</td>
                        ...
                        <td class="text-end py-1" data-kt-filemanager-table="action_dropdown">
                            <div class="d-flex justify-content-end">
                                ...
                                <div class="ms-2">
                                    @can('edit-news')
                                    @if($news->type == 'article')
                                        <a href="{{ route('admin.link.edit', $news->id) }}" class="btn btn-sm btn-icon btn-light-info" target="_self" title="পরিবর্তন করুন">
                                            <i class="ki-solid ki-feather fs-3 m-0">
                                            </i>
                                        </a>
                                    @elseif($news->type == 'pdf' || $news->type == 'image' || $news->type == 'url')
                                    ...
                                    @endif
                                    <form 
                                    action="{{ route('admin.link.visibility', $news->id) }}"  
                                    class="btn btn-sm btn-icon 
                                    {{ $news->visibility ? 'btn-light-primary' : 'btn-secondary' }}"  
                                    method="POST"
                                    onclick="this.submit()"
                                    title="{{ $news->visibility ? 'ওয়েবসাইট থেকে সরিয়ে ফেলুন' : 'ওয়েবসাইটে যোগ করুন' }}"
                                    >
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    @can('delete-news')
                                    <form 
                                    action="{{ route('admin.link.destroy', $news->id) }}"  
                                    class="btn btn-sm btn-icon btn-light-danger" 
                                    method="POST"
                                    onclick="submitForm(this, event, '{{ $news->name }}')"
                                    title="ডিলিট করুন"
                                    >
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="modal-content rounded">
                ...
                <div class="modal-header pb-3 mb-8 border-0 justify-content-between bg-light-dark">
                    ...
                    <div class="text-center">
                       <!--begin::Title-->
                       <h3 class="text-gray-800 fw-semibold fs-4">নতুন বার্তা</h3>
                       <!--end::Title-->
                   </div>
                   <!--end::Heading-->
                   <!--begin::Close-->
